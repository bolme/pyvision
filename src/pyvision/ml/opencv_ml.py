...
def boost(data,responses,weak_count=100,max_depth=20,boost_type=cv2.BOOST_DISCRETE,weight_trim_rate=0.5):
    '''
    Auto trains an OpenCV SVM.
    '''
    np.float32(data)
    np.float32(responses) 
    params = dict(boost_type=boost_type,weak_count=weak_count,max_depth=max_depth,weight_trim_rate=weight_trim_rate)
    model = cv2.Boost()
    model.train(data,cv2.CV_ROW_SAMPLE,responses,params=params)
    return model
